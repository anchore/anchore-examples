...
1. **Install `anchorectl`** — Downloads and installs `anchorectl` from the Anchore release mirror. Configuration for `anchorectl` is stored as CI/CD variables. See Variables section.
...
| `IMAGE_TAG` | Pipeline file | Automatically set to `$CI_PIPELINE_ID`. Update to match your tagging strategy. |
